    ...
    def calculate_camera_position_multiple(self, corners_list, marker_size, camera_matrix, dist_coeffs):
        ...
        for corners in corners_list:
            try:
                ...
                camera_pos_weights.append(cvec)

            except Exception as e:
                print(f"Error processing a marker: {e}")

        # Calculate and print results
        num_detected_tags = len(camera_pos_weights)
        if camera_pos_weights:
            avg_camera_pos = None
            # weight for point 1 is 1/d_1 / (1/d_1 + 1/d_2 + 1/d_3), etc.
            for n in range(0, len(camera_pos_weights)):
                d_n = 1/np.linalg.norm(n-np.array(0,0,0))
                d_j_s = 0
                for j in range (0, len(camera_pos_weights)):
                    d_j = np.linalg.norm(n-np.array(0,0,0))
                    d_j_s += 1/d_j
